// BIP39-compliant mnemonic implementation (English)
// Uses OpenSSL PKCS5_PBKDF2_HMAC for seed derivation (PBKDF2-HMAC-SHA512)
...
"excite","exclude","excuse","execute","exercise","exhaust","exhibit","exist","exit","exotic",
...
"industry","infant","inflict","inform","inherit","initial","inject","injury","inmate","inner",
...
"wall","walnut","want","ward","warm","warrior","wash","wasp","waste","water",
...
static const size_t BIP39_WORD_COUNT = 2048;

static bool derive_seed_from_mnemonic(const std::string& mnemonic, const std::string& passphrase, std::vector<uint8_t>& out_seed) {
    std::string salt = "mnemonic" + passphrase;
    out_seed.assign(64, 0);
    // Use PKCS5_PBKDF2_HMAC with SHA512, 2048 iterations
    if (PKCS5_PBKDF2_HMAC(mnemonic.c_str(), (int)mnemonic.size(),
                          (const unsigned char*)salt.c_str(), (int)salt.size(),
                          2048, EVP_sha512(), 64, out_seed.data()) != 1) {
        return false;
    }
    return true;
}
...
std::string generateMnemonic12() {
    ...
    if (shawncoin_secure_rand(entropy, 16) != 1)
        return std::string();
    // compute checksum (first ENT/32 bits of SHA256(entropy))
    unsigned char hash[SHA256_DIGEST_LENGTH];
    SHA256(entropy, 16, hash);
    // build bit sequence: entropy bits + checksum bits (4 bits for 128-bit entropy)
    std::vector<int> bits;
    bits.reserve(132);
    for (int i = 0; i < 16; ++i) {
        for (int j = 7; j >= 0; --j) bits.push_back((entropy[i] >> j) & 1);
    }
    // checksum length = ENT / 32 = 4 bits
    for (int i = 0; i < 4; ++i) bits.push_back((hash[0] >> (7 - i)) & 1);
...
std::vector<uint8_t> mnemonicToSeed(const std::string& mnemonic, const std::string& passphrase) {
    ...
    if (!derive_seed_from_mnemonic(mnemonic, passphrase, seed)) return {};
    return seed;
}
